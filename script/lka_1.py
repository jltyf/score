...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        if ego_speed == 60 / 3.6:
            xml_score_detail = f'{item}.测试车初始速度等于60km/h,得2分;<br/>'
            score += 2
        else:
            xml_score_detail = f'{item}.测试车初始速度不等于60km/h,不得分;<br/>'
        item += 1
        if ego_direction == math.radians(355):
            score += 2
            xml_score_detail = xml_score_detail + f'{item}.测试车初始车头方向偏离车道-5°,得2分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.测试车初始车头方向偏离车道不等于-5°,不得分;<br/>'
        item += 1
