...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        item += 1
        if ego_direction == math.radians(90):
            score += 1
        ...
        if veh_speed == 40 / 3.6:
            score += 1
            xml_score_detail = xml_score_detail + f'{item}.障碍车初速度等于40km/h,得1分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.障碍车初速度不等于40km/h,不得分;<br/>'
        item += 1
        if abs(distance) == 150:
            score += 1
