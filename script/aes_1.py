...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        if ego_speed == 60 / 3.6 and ego_direction == math.radians(90) and ego_car_type == 'CICV_Car':
        ...
        if 150 <= abs(distance) <= 200 and veh_speed == 0 and veh_car_type == 'Audi_A3_2009_blue':
            score += 1
            xml_score_detail = xml_score_detail + f'{item}.在同一车道上,测试车前方150-200m有车辆(Car_1),车型为Audi_A3_2009_blue,处于静止状态,得1分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.不满足在同一车道上,测试车前方150-200m有车辆(Car_1),车型为Audi_A3_2009_blue,处于静止状态,不得分;<br/>'
        item += 1
...
if __name__ == '__main__':
    score = get_score('/home/tang/xml/test/real/aes_1.xml', Weather.SNOW)
    print(score)
