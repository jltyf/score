...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        if ego_speed == 60 / 3.6 and ego_direction == math.radians(5):
            score += 2
            xml_score_detail = f'{item}.测试车初始速度等于60km/h且车头方向偏离车道5°,得2分;<br/>'
        else:
            xml_score_detail = f'{item}.不满足测试车初始速度等于60km/h且车头方向偏离车道5°,不得分;<br/>'
        item += 1
        if veh_speed == 40 / 3.6 and 1129.64 <= veh_start_x <= 1131.59 and 225 <= veh_start_y <= 325 and veh_acc == 3 and veh_acc_target == 0:
            score += 2
            xml_score_detail = xml_score_detail + f'{item}.障碍车位于测试车出弯位置前方100-200m范围内且在同一车道且以初速度40km/h，减速度3m/s²减速至静止,得2分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.不满足障碍车位于测试车出弯位置前方100-200m范围内且在同一车道且以初速度40km/h，减速度3m/s²减速至静止,不得分;<br/>'
        item += 1
