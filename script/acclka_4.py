...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        item += 1
        if veh_speed == 0 and 1129.64 <= veh_start_x <= 1131.59 and 175 <= veh_start_y <= 225:
            score += 2
            xml_score_detail = xml_score_detail + f'{item}.障碍车位于出测试车弯位置前方50-100m范围内且在同一车道且处于静止状态,得2分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.不满足障碍车位于出测试车弯位置前方50-100m范围内且在同一车道且处于静止状态,不得分;<br/>'
        item += 1
