...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        if ego_speed == 80 / 3.6 and ego_direction == math.radians(355) and ego_car_type == 'CICV_Car':
            score += 1
            xml_score_detail = f'{item}.测试车(Ego)车型为CICV_Car,以80km/h,初始车头方向偏离车道-5°,沿直道定速行驶,且不驶出本车道,得1分;<br/>'
        else:
            xml_score_detail = f'{item}.不满足测试车(Ego)车型为CICV_Car,以80km/h,初始车头方向偏离车道-5°,沿直道定速行驶,且不驶出本车道,不得分;<br/>'
        item += 1
        if 9.99 / 3.6 <= veh_speed <= 70.01 / 3.6 and 150 <= abs(distance) <= 200 and veh_car_type == 'Audi_A3_2009_red':
            score += 1
            xml_score_detail = xml_score_detail + f'{item}.测试车前方150-200m有车辆(veh_1),车型为Audi_A3_2009_red,以10-70km/h的速度定速行驶,得1分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.不满足测试车前方150-200m有车辆(veh_1),车型为Audi_A3_2009_red,以10-70km/h的速度定速行驶,不得分;<br/>'
        item += 1
...
if __name__ == '__main__':
    score = get_score('/home/tang/xml/pro/real/acclka_1.xml', Weather.SNOW)
    print(score)
