...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        if ego_speed == 60 / 3.6 and ego_direction == math.radians(5):
            score += 2
            xml_score_detail = f'{item}.测试车以60km/h且初始车头方向偏离车道5°,得2分;<br/>'
        else:
            xml_score_detail = f'{item}.不满足测试车以80km/h且初始车头方向偏离车道5°,不得分;<br/>'
        item += 1
        if veh_speed == 0 and abs(distance) == 250:
            score += 2
            xml_score_detail = xml_score_detail + f'{item}.障碍车处于静止状态且位于测试车前方250m,得2分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.不满足障碍车处于静止状态且位于测试车前方250m,不得分;<br/>'
        item += 1
