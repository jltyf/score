...
def get_score(xml_path, weather_result):
    ...
    try:
        ...
        if ego_speed == 60 / 3.6 and ego_direction == math.radians(5) and ego_car_type == 'CICV_Car':
            score += 1
            xml_score_detail = f'{item}.测试车(Ego)车型为CICV_Car,以60km/h,初始车头方向偏离车道5°,沿直道行驶,且不驶出本车道,得1分;<br/>'
        else:
            xml_score_detail = f'{item}.不满足测试车(Ego)车型为CICV_Car,以60km/h,初始车头方向偏离车道5°,沿直道行驶,且不驶出本车道,不得分;<br/>'
        item += 1
        if veh_speed == 0 and 150 <= abs(distance) <= 200 and veh_car_type == 'Audi_A3_2009_red':
            score += 1
            xml_score_detail = xml_score_detail + f'{item}.测试车前方150-200m有车辆(veh_1),车型为Audi_A3_2009_red,处于静止状态,得1分;<br/>'
        else:
            xml_score_detail = xml_score_detail + f'{item}.不满足测试车前方150-200m有车辆(veh_1),车型为Audi_A3_2009_red,处于静止状态,不得分;<br/>'
        item += 1
...
if __name__ == '__main__':
    score = get_score('/home/tang/xml/pro/real/aeblka_2.xml', Weather.RAIN)
    print(score)
